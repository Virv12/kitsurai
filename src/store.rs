...
pub(crate) fn table_create(name: &str) -> Result<()> {
...
pub(crate) fn table_delete(name: &str) -> Result<()> {
    let mut data = DATA.lock().unwrap();
...
pub(crate) fn item_get(table: &str, key: &str) -> Result<Option<Vec<u8>>> {
...
pub(crate) fn item_set(table: &str, key: &str, value: Vec<u8>) -> Result<()> {
    let mut data = DATA.lock().unwrap();
...
pub(crate) fn item_delete(table: &str, key: &str) -> Result<()> {
    let mut data = DATA.lock().unwrap();
    let Some(table) = data.get_mut(table) else {
        bail!("table does not exist: {}", table);
    };
